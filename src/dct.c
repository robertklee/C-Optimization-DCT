...
void run_dct(const int8_t *data, long width, long height, void (*dct_func)(const int8_t[8][8], int8_t[8][8]) )
{
    long width_strides = width / 8;
    long height_strides = height / 8;
    for (int i = 0; i < height_strides; ++i) {
        for (int j = 0; j < width_strides; ++j) {
            int8_t image_region[8][8];
            int8_t dct_output_region[8][8];
            // copy image into tmp
            for (int k = 0; k < 8; ++k) {
                for (int l = 0; l < 8; ++l) {
...
void copy_data(const int8_t data_in[8][8], int8_t data_out[8][8])
{
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
...
void dct(int8_t *data, long width, long height)
{
