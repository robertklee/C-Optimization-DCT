...
void transpose_int16_t(int16_t data[8][8])
{
    for (int i = 0; i < 8; ++i) {
        for (int j = i + 1; j < 8; ++j) {
            int16_t tmp = data[i][j];
            data[i][j] = data[j][i];
            data[j][i] = tmp;
        }
    }
...
void butterfly(int16_t *top, int16_t *bot, uint8_t type)
{
    int16_t tmp_val, tmp_top;
    switch (type)
    {
    case 1:
        /* code */
...
void dct_1d_int16_t(int16_t data[8])
{
    int16_t tmp_val; // extra temporary value
    // STAGE 1
    ...
    int16_t tmp_2 = data[7]; // x[2] -> X[2] // TODO: add some sort of temporary thing s.t. output isn't overwritten.
